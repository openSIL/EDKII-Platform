...
def set_build_env(env):
    ...
    env["SIL_PLATFORM_NAME"] = arguments.args.selected_build_attributes["board"]

def set_build_defines(command):
    command.extend(
        ["-D", "SIL_PLATFORM_NAME=" + arguments.args.selected_build_attributes["board"]]
    )
    command.extend(
        ["-D", "SOC_SKU=" + arguments.args.selected_build_attributes["soc_sku"]]
    )
